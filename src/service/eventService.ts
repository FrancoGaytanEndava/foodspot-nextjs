 ...
 */
export async function subscribeToAnEvent(
  userId: string,
  eventId: string,
  signal?: AbortSignal
): Promise<EventByIdResponse> {
 ...
 */
export async function unsubscribeToAnEvent(
  userId: string,
  eventId: string,
  signal?: AbortSignal
): Promise<EventByIdResponse> {
...
export async function getMembersAndReceiptsInfo(
  eventId: string,
  signal?: AbortSignal
): Promise<ITransferReceiptInfoResponse[]> {
...
/* export async function getMembersAmount(eventId: string, signal?: AbortSignal): Promise<PayCheckInfoResponse[]> {
	...
	return await _get(url, signal);
} descomentala cuando la necesites, por ahora no se usa
 */
